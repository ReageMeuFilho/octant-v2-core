...
abstract contract TokenizedStrategy {
     ...
     *
     *      CRITICAL: This struct uses a custom storage slot (BASE_STRATEGY_STORAGE) to ensure
     *      no collisions when using delegatecall from strategies. When extending strategies,
     *      NEVER add state variables outside this struct - extend the struct or use custom slots.
     *
     *      STORAGE EFFICIENCY:
    ...
    ///
    ///      CALCULATION:
    ///      keccak256("octant.base.strategy.storage") - 1
    ///      Subtracting 1 is an EIP-1967 pattern to avoid accidental collisions
    ///
    ///      SAFETY:
     ...
     *
     *      CONVERSION:
     *      - Uses Floor rounding (favors strategy, user gets slightly fewer shares)
     *      - shares = (assets * totalSupply) / totalAssets
     *      - First deposit: 1:1 ratio
     *
     *      SPECIAL VALUES:
     *      - assets = type(uint256).max: Deposits caller's full asset balance
     *
     *      CHECKS:
     *      - Amount <= maxDeposit (also checks if strategy shutdown)
     *      - Shares != 0 (prevents rounding to zero)
     *
     *      FLOW:
     *      1. Transfer assets from caller to strategy
     *      2. Update totalAssets
     *      3. Mint shares to receiver
     *      4. Emit Deposit event
     *
     * @param assets Amount of assets to deposit in asset base units (or type(uint256).max for full balance)
     ...
     *
     *      CONVERSION:
     *      - Uses Ceil rounding (favors strategy, user deposits slightly more)
     ...
     *
     *      CONVERSION:
     *      - Uses Ceil rounding (favors strategy, burns slightly more shares)
     *      - shares = (assets * totalSupply) / totalAssets + 1
     *
     *      LOSS HANDLING:
     *      - If strategy can't provide full assets (e.g., locked in yield source)
     *      - Realized loss must be <= (assets * maxLoss) / MAX_BPS
     *      - maxLoss = 0: No loss accepted (default, reverts on any loss)
     *      - maxLoss = 100: Accept up to 1% loss
     *      - maxLoss = 10000: Accept any loss (100%)
     *
     *      ALLOWANCE:
    ...
    /**
     * @notice Redeems exact shares for assets with loss tolerance
     * @dev ERC4626-extended redeem with loss parameter and reentrancy protection
     *
     *      CONVERSION:
     *      - Uses Floor rounding (favors strategy, user gets slightly less assets)
     *      - assets = (shares * totalAssets) / totalSupply
     *
     *      LOSS HANDLING:
     *      - Same as withdraw(): maxLoss caps acceptable loss
     *      - Default maxLoss = MAX_BPS (100%, accepts any loss)
     *      - Returns actual assets withdrawn (may be less than preview if loss)
     *
     ...
     *      Updated during deposits, withdrawals, and report() calls
     * @return totalAssets_ Total assets in asset base units (typically 18 decimals)
     */
     ...
     * @return totalSupply_ Total shares in share base units (typically 18 decimals)
     */
     ...
     * @dev Uses Floor rounding (favors strategy)
     ...
     * @dev Uses Ceil rounding (favors strategy)
     ...
     * @return max Maximum deposit amount in asset base units
     */
