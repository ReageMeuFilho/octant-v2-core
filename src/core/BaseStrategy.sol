...
abstract contract BaseStrategy {
    ...
    ///      bool shutdown = TokenizedStrategy.isShutdown();
    ///
     ...
     *
     *      EXAMPLE IMPLEMENTATION (Aave):
     *      ```solidity
     *      function _deployFunds(uint256 _amount) internal override {
     *          if (_amount == 0) return;
     *
     *          // Approve Aave pool
     *          asset.safeApprove(address(aavePool), _amount);
     *
     *          // Deposit and receive aTokens
     *          aavePool.deposit(address(asset), _amount, address(this), 0);
     *      }
     *      ```
     *
     * @param _amount Amount of asset to deploy (in asset base units)
     */
     ...
     *          uint256 toWithdraw = Math.min(_amount, available);
     *
     *          // Withdraw from Aave
     *          aavePool.withdraw(address(asset), toWithdraw, address(this));
     *
     *          // If we couldn't get full amount, user takes loss
     *          // (checked by TokenizedStrategy against maxLoss tolerance)
     *      }
     *      ```
     *
     * @param _amount Amount of asset needed (in asset base units)
     */
     ...
     *
     *      EXAMPLE IMPLEMENTATION (Aave with AAVE rewards):
     *      ```solidity
     *      function _harvestAndReport() internal override returns (uint256) {
     *          // 1. Harvest pending AAVE rewards
     ...
     *          }
     *
     *          // 2. Swap AAVE rewards to base asset
     *          uint256 aaveBalance = aave.balanceOf(address(this));
     *          if (aaveBalance > minAmountToSell) {
     *              _swapFrom(address(aave), address(asset), aaveBalance);
     *          }
     *
     *          // 3. Get current deployed position value
     *          uint256 deployed = aToken.balanceOf(address(this));
     *
     *          // 4. Get idle assets
     *          uint256 idle = asset.balanceOf(address(this));
     *
     *          // 5. Redeploy idle if not shutdown
     *          if (idle > 0 && !TokenizedStrategy.isShutdown()) {
     *              _deployFunds(idle);
     *              deployed += idle;
     *              idle = 0;
     *          }
     *
     *          // 6. Return accurate total
     ...
     *          // Deploy idle if above threshold (avoids sandwich)
     ...
     *      ```
     *
     * @param _totalIdle Current idle funds available to deploy (in asset base units)
     */
     ...
     *          // Tend when idle exceeds 10% of total assets
     *          uint256 idle = asset.balanceOf(address(this));
     *          return idle > TokenizedStrategy.totalAssets() / 10;
     *      }
     *      ```
     *
     ...
     *          uint256 deployed = aToken.balanceOf(address(this));
     *          uint256 toWithdraw = Math.min(_amount, deployed);
     *
     *          if (toWithdraw > 0) {
     *              aavePool.withdraw(address(asset), toWithdraw, address(this));
     *          }
