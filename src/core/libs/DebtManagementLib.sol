...
library DebtManagementLib {
    ...
    struct UpdateDebtResult {
        /// @notice New debt amount for the strategy (in asset base units)
        uint256 newDebt;
        /// @notice New total idle amount for the vault (in asset base units)
        uint256 newTotalIdle;
        /// @notice New total debt across all strategies (in asset base units)
    ...
    struct UpdateDebtVars {
        /// @notice Target debt we want strategy to have (in asset base units)
        uint256 newDebt;
        /// @notice Current debt strategy has (in asset base units)
        uint256 currentDebt;
        /// @notice Amount to withdraw if reducing debt (in asset base units)
        uint256 assetsToWithdraw;
        /// @notice Amount to deposit if increasing debt (in asset base units)
        uint256 assetsToDeposit;
        /// @notice Minimum amount vault must keep idle (in asset base units)
        uint256 minimumTotalIdle;
        /// @notice Current idle in vault (in asset base units)
        uint256 totalIdle;
        /// @notice Available idle after reserving minimum (in asset base units)
        uint256 availableIdle;
        /// @notice Max withdrawable from strategy per maxRedeem (in asset base units)
        uint256 withdrawable;
        /// @notice Max deposit to strategy per maxDeposit (in asset base units)
        uint256 maxDeposit;
        /// @notice Strategy's maximum debt limit (in asset base units)
        uint256 maxDebt;
     ...
     * @param totalIdle Current vault idle assets (in asset base units)
     * @param totalDebt Current total debt across all strategies (in asset base units)
     * @param strategy Strategy address to rebalance
     * @param targetDebt Target debt for strategy (in asset base units, or type(uint256).max for max)
     * @param maxLoss Maximum acceptable loss in basis points (0-10000)
     * @param minimumTotalIdle Minimum idle to maintain in vault (in asset base units)
