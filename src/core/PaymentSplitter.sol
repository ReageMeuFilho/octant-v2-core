 ...
 * @notice Splits ETH and ERC20 token payments proportionally among payees
 * @dev Modified from OpenZeppelin to use initializable pattern instead of constructor
 *
 *      CORE FUNCTIONALITY:
 *      - Splits received payments proportionally based on share allocation
 *      - Supports both ETH and ERC20 tokens
 *      - Pull payment model (payees must claim their share)
 *      - Immutable payee list (set once during initialization)
 *
 *      DISTRIBUTION MODEL:
 *      - Each payee is assigned a number of shares
 *      - Payment distribution: (payeeShares / totalShares) * totalReceived
 *      - Shares can be equal (all get same amount) or arbitrary proportion
 *      - Example: Alice 50 shares, Bob 30 shares, Charlie 20 shares = 50%, 30%, 20% split
 *
 *      PULL PAYMENT:
 *      - Payments are NOT auto-forwarded
 *      - Funds accumulate in this contract
 *      - Payees call release() to claim their share
 *      - Prevents out-of-gas issues with many payees
 *
 *      LIMITATIONS:
 *      - Payee list is immutable (set during initialize, cannot be changed)
 *      - Not compatible with rebasing tokens (e.g., stETH, aTokens)
 *      - Not compatible with fee-on-transfer tokens (e.g., some exotic ERC20s)
 *      - Precision loss possible with very small shares
 *
 * @custom:modified-from OpenZeppelin Contracts v4.x
 * @custom:security Test thoroughly before sending real value - token compatibility varies
 */
contract PaymentSplitter is Initializable, Context {
    ...
    /// @param shares Number of shares assigned to the payee
    ...
    /// @param to Address receiving the payment
    /// @param amount Amount of tokens released (in token base units)
     ...
     * @param shares_ Array of shares for each payee (must all be > 0)
     ...
     * @return total Total tokens released in token base units
     */
     ...
     * @param token Address of the ERC20 token contract
     * @param account Address of the payee
     * @return amount Tokens released in token base units
     */
     ...
     * @param token Address of the ERC20 token contract
     * @param account Address of the payee
     * @return amount Claimable tokens in token base units
     */
    ...
    /**
     * @dev Adds a new payee during initialization
     * @dev Validates account and shares, updates global state
