...
contract TokenizedAllocationMechanism {
    ...
    /// @notice Voting delay must be greater than zero (seconds)
    error ZeroVotingDelay();
    /// @notice Voting period must be greater than zero (seconds)
    error ZeroVotingPeriod();
    /// @notice Quorum shares must be greater than zero (18 decimals)
    error ZeroQuorumShares();
    /// @notice Timelock delay must be greater than zero (seconds)
    error ZeroTimelockDelay();
    /// @notice Grace period must be greater than zero (seconds)
    error ZeroGracePeriod();
    /// @notice Start block must be set (non-zero)
    error ZeroStartBlock();
    /// @notice Provided start time is in the past
    ...
    /// @notice ERC20 name must be non-empty
    error EmptyName();
    /// @notice ERC20 symbol must be non-empty
    error EmptySymbol();
    /// @notice Registration of the given user is blocked by strategy hook
    /// @param user Address attempting to register
    error RegistrationBlocked(address user);
    /// @notice Voting already ended
    /// @param currentTime Current block timestamp in seconds
    /// @param endTime Voting end timestamp in seconds
    error VotingEnded(uint256 currentTime, uint256 endTime);
    /// @notice User already registered
    /// @param user Address that is already registered
    error AlreadyRegistered(address user);
    /// @notice Deposit exceeds maximum allowed
    ...
    /// @notice Voting power exceeds maximum allowed
    /// @param votingPower Voting power in shares units (18 decimals)
    /// @param maxAllowed Maximum allowed voting power (18 decimals)
    error VotingPowerTooLarge(uint256 votingPower, uint256 maxAllowed);
    /// @notice Deposit amount is below required minimum
    /// @param deposit Deposit amount in asset base units (token decimals)
    error InsufficientDeposit(uint256 deposit);
    /// @notice Proposer is not allowed by strategy hook
    /// @param proposer Address attempting to propose
    error ProposeNotAllowed(address proposer);
    /// @notice Recipient address is invalid (e.g., zero)
    /// @param recipient Invalid recipient address
    error InvalidRecipient(address recipient);
    /// @notice Invalid user address (e.g., zero)
    /// @param user Invalid user address
    error InvalidUser(address user);
    /// @notice Recipient already used for a live proposal
    /// @param recipient Recipient with an active proposal
    error RecipientUsed(address recipient);
    /// @notice Provided recipient does not match expected recipient for proposal
    ...
    /// @notice Provided description does not match expected description
    /// @param pid Proposal id
    error DescriptionMismatch(uint256 pid);
    /// @notice Description must be non-empty
    error EmptyDescription();
    /// @notice Description length exceeds maximum
    ...
    /// @notice Voting has not ended yet
    /// @param currentTime Current block timestamp in seconds
    /// @param endTime Voting end timestamp in seconds
    error VotingNotEnded(uint256 currentTime, uint256 endTime);
    /// @notice Vote tally has already been finalized
    error TallyAlreadyFinalized();
    /// @notice Finalization was blocked by strategy hook
    error FinalizationBlocked();
    /// @notice Vote tally not yet finalized
    error TallyNotFinalized();
    /// @notice Proposal id does not reference a valid proposal
    /// @param pid Invalid proposal id
    error InvalidProposal(uint256 pid);
    /// @notice Proposal has been canceled
    /// @param pid Canceled proposal id
    error ProposalCanceledError(uint256 pid);
    /// @notice Quorum not met for proposal
    /// @param pid Proposal id
    /// @param forVotes Total for votes (18 decimals)
    /// @param againstVotes Total against votes (18 decimals)
    /// @param required Quorum threshold in shares (18 decimals)
    error NoQuorum(uint256 pid, uint256 forVotes, uint256 againstVotes, uint256 required);
    /// @notice Proposal already queued
    /// @param pid Proposal id
    error AlreadyQueued(uint256 pid);
    /// @notice Cannot queue after redemptions are enabled
    error QueueingClosedAfterRedemption();
    /// @notice No allocation (shares to mint is zero)
    /// @param pid Proposal id
    /// @param sharesToMint Calculated shares to mint (18 decimals)
    error NoAllocation(uint256 pid, uint256 sharesToMint);
    /// @notice Insufficient assets available for requested redemption/transfer
    ...
    /// @notice Voting window closed for this operation
    /// @param currentTime Current block timestamp in seconds
    ...
    /// @notice Vote weight is invalid relative to voting power
    /// @param weight Vote weight in shares (18 decimals)
    /// @param votingPower Voter's voting power in shares (18 decimals)
    error InvalidWeight(uint256 weight, uint256 votingPower);
    /// @notice Vote weight exceeds allowed maximum
    /// @param weight Vote weight in shares (18 decimals)
    /// @param maxAllowed Maximum allowed weight in shares (18 decimals)
    error WeightTooLarge(uint256 weight, uint256 maxAllowed);
    /// @notice Voting power increased between signature and execution
    /// @param oldPower Previous voting power in shares (18 decimals)
    /// @param newPower New voting power in shares (18 decimals)
    error PowerIncreased(uint256 oldPower, uint256 newPower);
    /// @notice Caller is not the original proposer
    /// @param caller Caller address
    /// @param proposer Expected proposer address
    error NotProposer(address caller, address proposer);
    /// @notice Proposal already canceled
    /// @param pid Proposal id
    error AlreadyCanceled(uint256 pid);
    /// @notice Caller lacks required authorization (onlyOwner)
    error Unauthorized();
     ...
     */
    struct Proposal {
        /// @notice Number of shares requested if proposal succeeds (18 decimals)
    ...
    struct AllocationStorage {
        ...
        uint256 gracePeriod;
        /// @notice Quorum threshold in shares required for success (18 decimals)
        ...
        /// @notice Share balances per account (18 decimals)
        mapping(address => uint256) balances; // Mapping to track current balances for each account that holds shares
        /// @notice Allowances mapping for share spenders (18 decimals)
        mapping(address => mapping(address => uint256)) allowances; // Mapping to track the allowances for the strategies shares
        /// @notice Total number of shares in circulation (18 decimals)
        ...
        /// @notice Voting power per user in shares (18 decimals)
        mapping(address => uint256) votingPower;
        /// @notice Shares allocated to each proposal (18 decimals)
    ...
    /// @param shareAmount Number of shares minted/allocated (18 decimals)
    ...
    /// @param shares Amount of shares burned (18 decimals)
    event Withdraw(
