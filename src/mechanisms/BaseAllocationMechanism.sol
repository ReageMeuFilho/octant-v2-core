...
abstract contract BaseAllocationMechanism is IBaseAllocationStrategy {
     ...
     *
     *      function _beforeSignupHook(address user) internal view override returns (bool) {
     ...
     *          return _tokenizedAllocation().votingPower(proposer) >= proposalThreshold;
     *      }
     *      ```
     *
     ...
     *          // Quadratic voting: sqrt(deposit) with precision scaling
     *          return Math.sqrt(deposit * 1e18);
     *      }
     *      ```
     *
     *      EXAMPLE (Linear with Cap):
     *      ```solidity
     *      uint256 constant MAX_VOTING_POWER = 1000e18;
     *
     *      function _getVotingPowerHook(address user, uint256 deposit)
     *          internal view override returns (uint256)
     *      {
     *          // Linear but capped to prevent whale dominance
     *          return Math.min(deposit, MAX_VOTING_POWER);
     *      }
     *      ```
     *
     ...
     *          return proposals[pid].id != 0; // Check proposal was created
     *      }
     *      ```
     *
     ...
     *
     *      EXAMPLE (One-time voting):
     *      ```solidity
     *      function _processVoteHook(\n     *          uint256 pid,\n     *          address voter,\n     *          VoteType choice,\n     *          uint256 weight,\n     *          uint256 oldPower\n     *      ) internal override returns (uint256) {\n     *          require(weight == oldPower, \"Must use full power\");\n     *          \n     *          // Record vote\n     *          if (choice == VoteType.For) {\n     *              proposals[pid].forVotes += weight;\n     *          }\n     *          \n     *          // Power fully consumed\n     *          return 0;\n     *      }\n     *      ```
     *
     ...
     *          return proposals[pid].forVotes >= quorumShares;
     *      }
     *      ```
     *
     ...
     *          uint256 votes = proposals[pid].forVotes;
          // QF: allocation proportional to sqrt of votes
          return (votes * votes) / totalVotes;\n     *      }
     *      ```
     * 
     ...
     *          return block.timestamp >= votingPeriodEnd;
     *      }
     *      ```
     *
