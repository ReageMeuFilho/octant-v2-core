...
contract QuadraticVotingMechanism is BaseAllocationMechanism, ProperQF {
    ...
    /// @notice Thrown when zero address attempts to propose
    error ZeroAddressCannotPropose();

    /// @notice Thrown when attempting Against or Abstain votes (only For supported)
    error OnlyForVotesSupported();

    /// @notice Thrown when voting power insufficient for quadratic cost
    error InsufficientVotingPowerForQuadraticCost();

    /**
     * @notice Thrown when voter has already voted on a proposal
     ...
     * @param pid Proposal ID
     * @return shares Total funding to allocate (in 18 decimals)
     */
    ...
    /// @param newNumerator The numerator of the new alpha value (dimensionless ratio)
    /// @param newDenominator The denominator of the new alpha value (dimensionless ratio)
