...
contract QuadraticVotingE2E is Test {
    ...
    function _calculateMatchingFunds( uint256 totalUserDeposits ) internal view returns (uint256 matchingFundsNeeded, uint256 totalQuadraticSum, uint256 totalLinearSum) {
    ...
    function testSetupVerification() public view {
        ...
        assertEq(_tokenized(address(mechanism)).votingPower(charlie), 0, "Charlie should have no voting power initially");
    ...
    function testUserSignup() public {
        ...
        assertEq(_tokenized(address(mechanism)).votingPower(alice), DEPOSIT_AMOUNT, "Alice should have voting power equal to deposit");
        assertEq(token.balanceOf(alice), INITIAL_TOKEN_BALANCE - DEPOSIT_AMOUNT, "Alice token balance should decrease by deposit amount");
        ...
        // Verify Bob's signup
        assertEq(_tokenized(address(mechanism)).votingPower(bob), bobDeposit, "Bob should have voting power equal to his deposit");
        assertEq(token.balanceOf(address(mechanism)), DEPOSIT_AMOUNT + bobDeposit, "Mechanism should have both deposits");
        ...
        assertEq(token.balanceOf(address(mechanism)), DEPOSIT_AMOUNT + bobDeposit, "Mechanism balance should be unchanged by zero deposit");
    }
    ...
    function testProposalCreation() public {
        ...
        assertEq(uint8(_tokenized(address(mechanism)).state(pid1)), uint8(TokenizedAllocationMechanism.ProposalState.Pending), "Proposal should be Pending");
    ...
    function testSignupAndProposalWorkflow() public {
        ...
        assertEq(uint8(_tokenized(address(mechanism)).state(pid1)), uint8(TokenizedAllocationMechanism.ProposalState.Pending), "Proposal 1 should be Pending");
        assertEq(uint8(_tokenized(address(mechanism)).state(pid2)), uint8(TokenizedAllocationMechanism.ProposalState.Pending), "Proposal 2 should be Pending");
        assertEq(uint8(_tokenized(address(mechanism)).state(pid3)), uint8(TokenizedAllocationMechanism.ProposalState.Pending), "Proposal 3 should be Pending");
        
        // Verify recipients are different
        assertTrue(p1.recipient != p2.recipient && p2.recipient != p3.recipient && p1.recipient != p3.recipient, "All recipients should be unique");
        
        // Verify mechanism state remains consistent
        assertEq(token.balanceOf(address(mechanism)), totalDeposits, "Mechanism balance should be unchanged by proposals");
    ...
    function testMultiUserVotingScenario() public {
        ...
        // Verify total funding allocation
        uint256 totalQuadraticFunding = p1Quadratic + p2Quadratic + p3Quadratic;
        uint256 totalLinearFunding = p1Linear + p2Linear + p3Linear;
        uint256 totalContributions = p1Contributions + p2Contributions + p3Contributions;
        
        console.log("Total quadratic funding:", totalQuadraticFunding);
        console.log("Total linear funding:", totalLinearFunding);
        console.log("Total contributions:", totalContributions);
        
        assertEq(totalQuadraticFunding, 2025 ether + 625 ether + 576 ether, "Total quadratic funding calculation");
        assertEq(totalLinearFunding, 0 ether, "Total linear funding should be 0 with alpha=1");
        assertEq(totalContributions, 1025 ether + 325 ether + 296 ether, "Total contributions calculation");
    ...
    function testMatchingFundsCalculationAnd1to1Ratio() public {
        ...
        (uint256 matchingFundsNeeded, uint256 totalQuadraticSum, uint256 totalLinearSum) = _calculateMatchingFunds(totalUserDeposits);
        ...
        assertEq(contractBalanceAfterMatching, totalQuadraticSum, "Contract should hold exactly the total quadratic sum after matching");
    ...
    function testOptimalAlphaCalculationWith1to1Ratio() public {
        ...
        // Update alpha to the calculated optimal value
        ...
        // Verify total assets = user deposits + matching pool
        ...
        // Calculate expected total funding with this alpha
        ...
        uint256 expectedTotalFunding = expectedQuadraticComponent + expectedLinearComponent;
