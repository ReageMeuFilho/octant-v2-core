...
contract QuadraticVotingCrossJourneyTest is Test {
    ...
    function testCompleteEndToEnd_Integration() public {
        ...
        assertTrue(_tokenized(address(mechanism)).votingPower(alice) < 1000 ether, "Alice should have consumed most voting power");
        assertTrue(_tokenized(address(mechanism)).votingPower(bob) < 1000 ether, "Bob should have consumed most voting power");
        assertTrue(_tokenized(address(mechanism)).votingPower(frank) < 1000 ether, "Frank should have consumed most voting power");
    }
    ...
    function testMultiProposal_ComplexScenarios() public {
        ...
        assertTrue(pid3State == uint8(TokenizedAllocationMechanism.ProposalState.Succeeded) || 
