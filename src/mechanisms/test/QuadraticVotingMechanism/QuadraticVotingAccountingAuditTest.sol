...
contract QuadraticVotingAccountingAuditTest is Test {
    ...
    function _verifyAccountingInvariants(AccountingState memory state, string memory phase) internal pure {
        ...
        if (keccak256(bytes(phase)) == keccak256(bytes("FINAL"))) {
            ...
            assertTrue(state.totalMechanismAssets <= 2, 
            ...
            assertTrue(state.totalMechanismAssets < originalAssets, 
            ...
            assertEq(state.totalMechanismAssets, expectedTotalAssets, 
        ...
        assertEq(totalIndividualShares, state.totalSharesSupply,
        ...
        assertTrue(totalVotingPower <= USER_DEPOSIT * 3,
            string(abi.encodePacked("Voting power overflow in ", phase)));
    }
    ...
    function testCompleteAccountingAudit_ThreeVotersTwoProposals() public {
        ...
        assertEq(postSignupState.totalMechanismAssets, USER_DEPOSIT * 3, 
            "Assets should include all user deposits");
        ...
        assertEq(postProposalState.totalMechanismAssets, postSignupState.totalMechanismAssets, 
            "Proposal creation should not affect assets");
        assertEq(postProposalState.aliceVotingPower, postSignupState.aliceVotingPower, 
        ...
        assertEq(postVotingState.charlieVotingPower, expectedRemainingPower, "Charlie voting power incorrectly consumed");
        ...
        assertTrue(postFinalizationState.totalMechanismAssets >= totalAssetsNeeded, 
        ...
        assertEq(postQueueingState.recipient1Shares, expectedSharesPerRecipient, "Recipient1 should get exactly 3600 shares");
        assertEq(postQueueingState.recipient2Shares, expectedSharesPerRecipient, "Recipient2 should get exactly 3600 shares");
        assertEq(postQueueingState.totalSharesSupply, expectedSharesPerRecipient * 2, "Total shares should be exactly 7200");
        
        // Verify timelock setup
        uint256 expectedRedeemableTime = queueTimestamp + TIMELOCK_DELAY;
        assertEq(_tokenized(address(mechanism)).redeemableAfter(recipient1), expectedRedeemableTime, 
            "Recipient1 timelock incorrect");
        assertEq(_tokenized(address(mechanism)).redeemableAfter(recipient2), expectedRedeemableTime, 
        ...
        uint256 recipient1AssetsReceived = _tokenized(address(mechanism)).redeem(
            recipient1ActualShares, recipient1, recipient1
        );
        ...
        assertEq(recipient1AssetsReceived, expectedRecipient1Assets, "Recipient1 assets must equal exact proportional share");
        assertEq(token.balanceOf(recipient1), recipient1BalanceBefore + recipient1AssetsReceived, 
            "Recipient1 token balance incorrect");
        assertEq(token.balanceOf(address(mechanism)), mechanismBalanceBefore - recipient1AssetsReceived, 
        ...
        uint256 recipient2AssetsReceived = _tokenized(address(mechanism)).redeem(
            recipient2ActualShares, recipient2, recipient2
        );
        ...
        assertEq(recipient2AssetsReceived, expectedRecipient2Assets, "Recipient2 assets must equal exact proportional share");
        assertEq(token.balanceOf(recipient2), recipient2BalanceBefore + recipient2AssetsReceived, 
        ...
        assertEq(recipient1AssetsReceived, recipient2AssetsReceived, 
    ...
    function testCapitalConstrainedAudit_FixedMatchingPool() public {
        ...
        uint256 project1LinearComponent = (1250 * (constrainedAlphaDenominator - constrainedAlphaNumerator)) / constrainedAlphaDenominator;
        ...
        uint256 project2LinearComponent = (369 * (constrainedAlphaDenominator - constrainedAlphaNumerator)) / constrainedAlphaDenominator;
        ...
        assertEq(finalState.totalMechanismAssets, totalAssets, "Total assets should equal user deposits + required matching pool");
        assertEq(totalShares, expectedProject1Funding + expectedProject2Funding, "Total shares should equal sum of project funding");
        ...
        uint256 totalActualSharesRedeemed = recipient1Shares - _tokenized(address(mechanism)).balanceOf(recipient1) +
                                           recipient2Shares - _tokenized(address(mechanism)).balanceOf(recipient2);
        ...
        assertTrue(_tokenized(address(mechanism)).balanceOf(recipient1) <= 1, "Recipient1 should have at most 1 share remaining");
        assertTrue(_tokenized(address(mechanism)).balanceOf(recipient2) <= 1, "Recipient2 should have at most 1 share remaining");
        ...
        if (remainingAssets > 0) {
            ...
            if (remainingShares > 0) {
                console.log("- Current exchange rate: remainingAssets/remainingShares =", remainingAssets, "/", remainingShares);
            }
        }
