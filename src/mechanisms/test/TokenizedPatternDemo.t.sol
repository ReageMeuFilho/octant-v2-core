...
contract TokenizedPatternDemoTest is Test {
    ...
    function testProposalCreationFailures() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            TokenizedAllocationMechanism.ProposeNotAllowed.selector, alice
        ));
        vm.prank(alice);
    ...
    function testBasicVoting() public {
        ...
        // Check vote tally
        (uint256 forVotes, uint256 againstVotes, uint256 abstainVotes) = _tokenized(address(mechanism)).getVoteTally(pid);
    ...
    function testSimpleVotingHooks() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            TokenizedAllocationMechanism.ProposeNotAllowed.selector, bob
        ));
        vm.prank(bob);
    ...
    function testCompleteVotingLifecycle() public {
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pid2)), uint(TokenizedAllocationMechanism.ProposalState.Defeated));
        ...
        vm.expectRevert(abi.encodeWithSelector(
            TokenizedAllocationMechanism.NoQuorum.selector, 
            pid2, 150 ether, 75 ether, 100 ether
        ));
        ...
        // Verify final states
        assertEq(uint(_tokenized(address(mechanism)).state(pid1)), uint(TokenizedAllocationMechanism.ProposalState.Queued));
        assertEq(uint(_tokenized(address(mechanism)).state(pid2)), uint(TokenizedAllocationMechanism.ProposalState.Defeated));
    }
