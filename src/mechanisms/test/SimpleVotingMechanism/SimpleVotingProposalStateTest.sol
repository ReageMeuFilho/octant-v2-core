...
contract SimpleVotingProposalStateTest is Test {
    ...
    function testProposalState_Defeated() public {
        ...
        vm.prank(alice);
        _tokenized(address(mechanism)).castVote(pidNegativeVotes, TokenizedAllocationMechanism.VoteType.Against, 400 ether);
        vm.prank(bob);
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pidLowVotes)), uint(TokenizedAllocationMechanism.ProposalState.Defeated));
        assertEq(uint(_tokenized(address(mechanism)).state(pidNegativeVotes)), uint(TokenizedAllocationMechanism.ProposalState.Defeated));
    ...
    function testProposalState_Succeeded() public {
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pid)), uint(TokenizedAllocationMechanism.ProposalState.Succeeded));
    ...
    function testProposalState_Expired() public {
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pid)), uint(TokenizedAllocationMechanism.ProposalState.Expired));
    ...
    function testRecipientJourney_MultipleProposalStates() public {
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pidSuccessful)), uint(TokenizedAllocationMechanism.ProposalState.Pending));
        assertEq(uint(_tokenized(address(mechanism)).state(pidDefeated)), uint(TokenizedAllocationMechanism.ProposalState.Pending));
        assertEq(uint(_tokenized(address(mechanism)).state(pidCanceled)), uint(TokenizedAllocationMechanism.ProposalState.Pending));
        
        // Move to ACTIVE
        vm.roll(startBlock + VOTING_DELAY + 1);
        assertEq(uint(_tokenized(address(mechanism)).state(pidSuccessful)), uint(TokenizedAllocationMechanism.ProposalState.Active));
        assertEq(uint(_tokenized(address(mechanism)).state(pidDefeated)), uint(TokenizedAllocationMechanism.ProposalState.Active));
        assertEq(uint(_tokenized(address(mechanism)).state(pidCanceled)), uint(TokenizedAllocationMechanism.ProposalState.Active));
        
        // Cancel one proposal
        vm.prank(alice);
        _tokenized(address(mechanism)).cancelProposal(pidCanceled);
        assertEq(uint(_tokenized(address(mechanism)).state(pidCanceled)), uint(TokenizedAllocationMechanism.ProposalState.Canceled));
        ...
        assertEq(uint(_tokenized(address(mechanism)).state(pidDefeated)), uint(TokenizedAllocationMechanism.ProposalState.Defeated));
        assertEq(uint(_tokenized(address(mechanism)).state(pidCanceled)), uint(TokenizedAllocationMechanism.ProposalState.Canceled));
