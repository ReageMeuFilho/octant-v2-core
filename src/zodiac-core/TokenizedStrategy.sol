...
/**
 * @title TokenizedStrategy (Zodiac Core Variant)
 * @author [Golem Foundation](https://golem.foundation)
 * @custom:security-contact security@golem.foundation
 * @notice Abstract implementation for Zodiac-integrated tokenized strategies
 * @dev Zodiac module variant of TokenizedStrategy with Gnosis Safe integration and Hats Protocol roles
 *
 *      KEY DIFFERENCES FROM CORE VERSION:
 *      - Zodiac Module: Integrates with Gnosis Safe via IAvatar
 *      - Hats Protocol: Role-based access control via Hats
 *      - Operator Role: Additional privileged role for Safe operations
 *      - Regen Governance: Separate governance for regenerative parameters
 *
 *      HATS PROTOCOL INTEGRATION:
 *      - Roles defined as hats (hierarchical access control)
 *      - Operator hat controls Safe module operations
 *      - Management hat controls strategy parameters
 *      - Regen governance hat controls donation settings
 *
 *      SAFE MODULE OPERATIONS:
 *      - Can execute transactions on Safe via IAvatar
 *      - Operator-gated for security
 *      - Enables automated strategy operations via Safe
 *
 * @custom:security Zodiac module pattern - integrates with Gnosis Safe
 * @custom:security Hats Protocol provides hierarchical role management
 */
