...
import { TokenizedYieldSkimmingStrategy, IBaseStrategy, Math } from "src/zodiac-core/vaults/TokenizedYieldSkimmingStrategy.sol";
...
contract DragonTokenizedYieldSkimmingStrategy is IDragonTokenizedStrategy, TokenizedYieldSkimmingStrategy {
    ...
    ) external override(TokenizedYieldSkimmingStrategy, ITokenizedStrategy) {
    ...
    function deposit(
        ...
        external
        payable
        override(TokenizedYieldSkimmingStrategy, IERC4626Payable)
    ...
    function maxWithdraw(
        address _owner
    ) external view override(TokenizedYieldSkimmingStrategy, IERC4626Payable) returns (uint256) {
    ...
    function maxWithdraw(
        address _owner,
        uint256 /*maxLoss*/
    ) external view override(TokenizedYieldSkimmingStrategy, ITokenizedStrategy) returns (uint256) {
        return _maxWithdraw(super._strategyStorage(), _owner);
    }
    ...
    function transfer(
        address,
        /*to*/ uint256 /*amount*/
    ) external pure override(TokenizedYieldSkimmingStrategy, IERC20) returns (bool) {
    ...
    /**
     * @inheritdoc TokenizedYieldSkimmingStrategy
     */
    function permit(
        address,
        address,
        ...
        bytes32,
        bytes32
    ) external pure override(TokenizedYieldSkimmingStrategy, IERC20Permit) {
        revert DragonTokenizedStrategy__VaultSharesNotTransferable();
    }
     ...
     * @inheritdoc IERC20
     */
    function approve(address, uint256) external pure override(TokenizedYieldSkimmingStrategy, IERC20) returns (bool) {
        revert DragonTokenizedStrategy__VaultSharesNotTransferable();
    }
    ...
    function withdraw(
        ...
        public
        virtual
        override(TokenizedYieldSkimmingStrategy, ITokenizedStrategy)
        nonReentrant
        returns (uint256 shares)
    {
    ...
    function redeem(
        ...
        address _owner,
        uint256 maxLoss
    ) public virtual override(TokenizedYieldSkimmingStrategy, ITokenizedStrategy) nonReentrant returns (uint256) {
        // Get the storage slot for all following calls.
        StrategyData storage S = super._strategyStorage();
        LockupInfo storage lockup = S.voluntaryLockups[_owner];
