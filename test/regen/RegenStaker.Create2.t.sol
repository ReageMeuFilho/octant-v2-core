...
contract RegenStakerCreate2Test is Test {
    ...
    function testDifferentDelegateesDifferentAddresses(
        address delegatee1,
        address delegatee2
    ) public {
    ...
    function testUsersWithDifferentDelegateesGetDifferentSurrogates() public {
        ...
        assertEq(stakeToken.balanceOf(aliceSurrogate), STAKE_AMOUNT, "Charlie's surrogate should only hold Alice's stake");
        ...
        assertEq(IERC20Delegates(address(stakeToken)).delegates(aliceSurrogate), charlieDelegate, "Alice's surrogate should delegate to Charlie");
        assertEq(IERC20Delegates(address(stakeToken)).delegates(bobSurrogate), davidDelegate, "Bob's surrogate should delegate to David");
    }
    ...
    function testFuzzMultipleUsersShareSurrogateForSameDelegatee(
        ...
        assertEq(IERC20Delegates(address(stakeToken)).delegates(expectedSurrogate), delegatee, "Surrogate should delegate to correct delegatee");
    }
    ...
    function testSameUserMultipleDepositsToSameDelegatee() public {
        ...
        // Verify the surrogate delegates to the correct delegatee
        assertEq(IERC20Delegates(address(stakeToken)).delegates(surrogate1), delegatee, "Surrogate should delegate to delegatee");
        
        // Verify deposits are separate
        assertTrue(Staker.DepositIdentifier.unwrap(depositId1) != Staker.DepositIdentifier.unwrap(depositId2) && 
    ...
    function testChangingDelegateeCreatesDifferentSurrogate() public {
        ...
        assertEq(IERC20Delegates(address(stakeToken)).delegates(surrogate1), delegatee1, "First surrogate should delegate to first delegatee");
        assertEq(IERC20Delegates(address(stakeToken)).delegates(surrogate2), delegatee2, "Second surrogate should delegate to second delegatee");
    }
