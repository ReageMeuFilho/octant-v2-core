...
abstract contract StakerTestBase is Test, PercentAssertions {
    ...
    function _boundToRealisticStake(uint256 _stakeAmount) public pure returns (uint256 _boundedStakeAmount) {
    ...
    function _assumeSafeDepositorAndSurrogate(address _depositor, address _delegatee) internal {
        ...
        vm.assume((!isKnownSurrogate[DelegationSurrogate(_depositor)]) && (!isKnownDepositor[address(_surrogate)]));
    }

    function _setClaimFeeAndCollector(uint96 _amount, address _collector) internal {
        Staker.ClaimFeeParameters memory _params = Staker.ClaimFeeParameters({
            feeAmount: _amount,
    ...
    function _stake(
        ...
        uint256 _amount,
        address _delegatee
    ) internal returns (Staker.DepositIdentifier _depositId) {
    ...
    function _fetchDeposit(Staker.DepositIdentifier _depositId) internal view returns (Staker.Deposit memory) {
        (
            uint96 _balance,
            address _owner,
