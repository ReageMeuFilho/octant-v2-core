...
import { Staker, IERC20, IEarningPowerCalculator } from "staker/Staker.sol";
...
import { MockStakerHarness, StakerDelegateSurrogateVotes, StakerPermitAndStake } from "./mocks/MockStakerHarness.sol";
...
contract StakerTest is StakerTestBase {
    ...
    function _deployStaker() public virtual override(StakerTestBase) returns (Staker _staker) {
        return new StakerHarness(rewardToken, govToken, earningPowerCalculator, maxBumpTip, admin, "Staker");
    }
...
contract Constructor is StakerTest {
    ...
    function testFuzz_RevertIf_StakeTokenMismatchBetweenStakerAndDelegateSurrogate(
        ...
        vm.expectRevert(StakerDelegateSurrogateVotes.StakerDelegateSurrogateVotes__UnauthorizedToken.selector);
...
contract Stake is StakerTest {
    ...
    function testFuzz_RevertIf_ClaimerIsTheZeroAddress(address _depositor, uint256 _amount, address _delegatee) public {
        vm.assume(_delegatee != address(0));
    ...
    function testFuzz_SetsScaledEarningPowerWhenCalculatorScalesStakeAmount(
        ...
        (, Staker.DepositIdentifier _depositId) = _boundMintAndStake(_depositor, _stakeAmount, _depositor);
...
contract PermitAndStake is StakerTest {
    ...
    function testFuzz_PerformsTheApprovalByCallingPermitThenPerformsStake(
        ...
        stdstore.target(address(govToken)).sig("nonces(address)").with_key(_depositor).checked_write(_currentNonce);
        ...
        bytes32 _messageHash = keccak256(abi.encodePacked("\x19\x01", govToken.DOMAIN_SEPARATOR(), _message));
        (uint8 _v, bytes32 _r, bytes32 _s) = vm.sign(_depositorPrivateKey, _messageHash);

        vm.prank(_depositor);
        Staker.DepositIdentifier _depositId = govStaker.permitAndStake(
            _depositAmount,
            _delegatee,
            _claimer,
            _deadline,
...
contract StakeMore is StakerTest {
    function testFuzz_TransfersStakeToTheExistingSurrogate(
        ...
        Staker.DepositIdentifier _depositId;
        (_depositAmount, _depositId) = _boundMintAndStake(_depositor, _depositAmount, _delegatee, _claimer);
        Staker.Deposit memory _deposit = _fetchDeposit(_depositId);
    ...
    function testFuzz_RevertIf_TheCallerIsNotTheDepositor(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not owner"), _notDepositor)
        );
        govStaker.stakeMore(_depositId, _addAmount);
    }
    ...
    function testFuzz_RevertIf_TheDepositIdentifierIsInvalid(
        ...
        vm.prank(_depositor);
        vm.expectRevert(abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not owner"), _depositor));
        govStaker.stakeMore(_depositId, _addAmount);
    }
...
contract PermitAndStakeMore is StakerTest {
    ...
    function testFuzz_PerformsTheApprovalByCallingPermitThenPerformsStakeMore(
        ...
        Staker.DepositIdentifier _depositId;
        (_initialDepositAmount, _depositId) = _boundMintAndStake(
            _depositor,
    ...
    function testFuzz_SuccessfullyStakeMoreWhenApprovalExistsAndPermitSignatureIsInvalid(
        ...
        assertEq(govStaker.depositorTotalEarningPower(_depositor), _initialDepositAmount + _stakeMoreAmount);
    }
    ...
    function _executePermitAndStakeMore(
        ...
        bytes32 _message = keccak256(
            abi.encode(PERMIT_TYPEHASH, _depositor, address(govStaker), _addAmount, _currentNonce, _deadline)
        );
...
contract AlterDelegatee is StakerTest {
    function testFuzz_AllowsStakerToUpdateTheirDelegatee(
        ...
        Staker.DepositIdentifier _depositId;
        (_depositAmount, _depositId) = _boundMintAndStake(_depositor, _depositAmount, _firstDelegatee, _claimer);
    ...
    function testFuzz_UpdatesDepositsEarningPower(
        ...
        earningPowerCalculator.__setEarningPowerAndIsQualifiedForDelegatee(_newDelegatee, _newEarningPower, true);
...
contract AlterClaimer is StakerTest {
    function testFuzz_AllowsStakerToUpdateTheirClaimer(
        ...
        Staker.DepositIdentifier _depositId;
        (_depositAmount, _depositId) = _boundMintAndStake(_depositor, _depositAmount, _delegatee, _firstClaimer);
    ...
    function testFuzz_UpdatesDepositsEarningPower(
        ...
        earningPowerCalculator.__setEarningPowerAndIsQualifiedForDelegatee(_delegatee, _newEarningPower, true);
    ...
    function testFuzz_RevertIf_TheCallerIsNotTheDepositor(
        ...
        vm.assume(_notDepositor != _depositor && _newClaimer != address(0) && _newClaimer != _firstClaimer);
...
contract Withdraw is StakerTest {
    ...
    function testFuzz_UpdatesTheTotalStakedWhenTwoAccountsWithdraw(
        ...
        uint256 _remainingDeposits = _depositAmount1 + _depositAmount2 - _withdrawalAmount1 - _withdrawalAmount2;
    ...
    function testFuzz_UpdatesAnAccountsTotalStakedWhenItWithdrawals(
        ...
        assertEq(govStaker.depositorTotalStaked(_depositor), _depositAmount1 + _depositAmount2 - _withdrawalAmount);
...
contract SetRewardNotifier is StakerTest {
    ...
    function testFuzz_EmitsEventWhenRewardNotifierIsSet(address _rewardNotifier, bool _isEnabled) public {
        vm.expectEmit();
    ...
    function testFuzz_RevertIf_TheCallerIsNotTheAdmin(
        ...
        vm.prank(_notAdmin);
        vm.expectRevert(abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not admin"), _notAdmin));
...
contract SetEarningPowerCalculator is StakerTest {
    function testFuzz_AllowsAdminToSetEarningPowerCalculator(address _newEarningPowerCalculator) public {
    ...
    function testFuzz_EmitsEventWhenEarningPowerCalculatorIsSet(address _newEarningPowerCalculator) public {
        vm.assume(_newEarningPowerCalculator != address(0));

        vm.expectEmit();
        emit Staker.EarningPowerCalculatorSet(address(govStaker.earningPowerCalculator()), _newEarningPowerCalculator);

        vm.prank(admin);
        govStaker.setEarningPowerCalculator(_newEarningPowerCalculator);
    }

    function testFuzz_RevertIf_TheCallerIsNotTheAdmin(address _notAdmin, address _newEarningPowerCalculator) public {
        vm.assume(_notAdmin != govStaker.admin());
...
contract SetMaxBumpTip is StakerTest {
    ...
    function testFuzz_RevertIf_TheCallerIsNotTheAdmin(address _caller, uint256 _newMaxBumpTip) public {
        ...
        vm.prank(_caller);
        vm.expectRevert(abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not admin"), _caller));
        govStaker.setMaxBumpTip(_newMaxBumpTip);
    }
}
...
contract SetClaimFeeParameters is StakerTest {
    ...
    function testFuzz_AllowsAdminToSetFeeAmountAndFeeCollectorToZero(
        ...
        Staker.ClaimFeeParameters memory _zeroParams = Staker.ClaimFeeParameters({
            feeAmount: 0,
    ...
    function testFuzz_RevertIf_FeeAmountIsMoreThanTheMaxClaimFee(Staker.ClaimFeeParameters memory _newParams) public {
        vm.assume(_newParams.feeCollector != address(0));
        _newParams.feeAmount = uint96(bound(_newParams.feeAmount, govStaker.MAX_CLAIM_FEE() + 1, type(uint96).max));
...
contract StakerRewardsTest is StakerTest {
    ...
    function _boundToRealisticReward(uint256 _rewardAmount) public pure returns (uint256 _boundedRewardAmount) {
    ...
    ) public pure returns (uint96 _boundedStakeAmount, uint256 _boundedRewardAmount) {
...
contract NotifyRewardAmount is StakerRewardsTest {
    ...
    function testFuzz_AllowsMultipleApprovedRewardNotifiersToNotifyOfRewards(
        ...
        uint256 _expectedRewardRate = (SCALE_FACTOR * (_amount1 + _amount2 + _amount3)) / govStaker.REWARD_DURATION();
    ...
    function testFuzz_RevertIf_CallerIsNotTheRewardNotifier(uint256 _amount, address _notNotifier) public {
        ...
        rewardToken.transfer(address(govStaker), _amount);
        vm.expectRevert(
            abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not notifier"), _notNotifier)
        );
        govStaker.notifyRewardAmount(_amount);
        vm.stopPrank();
    }
...
contract BumpEarningPower is StakerRewardsTest {
    function testFuzz_BumpsTheDepositsEarningPowerUp(
        ...
        (, Staker.DepositIdentifier _depositId) = _boundMintAndStake(_depositor, _stakeAmount, _delegatee);
        // The contract is notified of a reward
        _mintTransferAndNotifyReward(_rewardAmount);
    ...
    function testFuzz_EarningPowerChangesAfterMultiplierUpdate(
        ...
        (, Staker.DepositIdentifier depositId) = _boundMintAndStake(_depositor, _stakeAmount, _depositor);
...
contract RewardPerTokenAccumulated is StakerRewardsTest {
    function testFuzz_ReturnsZeroIfThereHasNeverBeenAReward(
        ...
        // Second deposit
        (, Staker.DepositIdentifier _depositId2) = _boundMintAndStake(_depositor2, _stakeAmount2, _depositor2);
        _jumpAheadByPercentOfRewardDuration(_durationPercent2);
    ...
    function testFuzz_AccruesTheCorrectValueWhenADepositorStakesForARewardDurationAndAnotherDepositorStakesForASecondRewardDuration(
        ...
        uint256 _expected = _scaledDiv(_rewardAmount, _stakeAmount) + _scaledDiv(_rewardAmount, 2 * _stakeAmount);
        assertLteWithinOnePercent(govStaker.rewardPerTokenAccumulated(), _expected);
    }
    ...
    function testFuzz_AccruesTheCorrectValueWhenTwoDepositorsStakeAtDifferentTimesAndThereAreTwoRewards(
        ...
        uint256 _expectedDuration1 = _percentOf(_scaledDiv(_rewardAmount, _stakeAmount), _durationPercent1);
        ...
        uint256 _expectedDuration2 = _percentOf(
            _scaledDiv(_firstRewardRemainingAmount + _rewardAmount, 2 * _stakeAmount),
            _durationPercent2
        );
    ...
    function testFuzz_AccruesTheCorrectValueWhenADepositorStakesAndWithdrawsDuringARewardDuration(
        ...
        uint256 _expectedDuration1 = _percentOf(_scaledDiv(_rewardAmount, _stakeAmount), _durationPercent1);
        uint256 _expectedDuration2 = _percentOf(
            _scaledDiv(_rewardAmount, _stakeAmount - _withdrawalAmount),
            _durationPercent2
        );
        uint256 _expected = _expectedDuration1 + _expectedDuration2;
        assertLteWithinOnePercent(govStaker.rewardPerTokenAccumulated(), _expected);
    ...
    function testFuzz_AccruesTheCorrectValueWhenTwoDepositorsStakeDifferentAmountsAtDifferentTimesOverTwoRewards(
        ...
        // proportional to the time elapsed
        uint256 _expectedDuration1 = _percentOf(_scaledDiv(_rewardAmount, _stakeAmount1), _durationPercent1);
        ...
        uint256 _scaledRewardsAfterDuration1 = (SCALE_FACTOR *
            _rewardAmount -
            _percentOf(SCALE_FACTOR * _rewardAmount, _durationPercent1)) + SCALE_FACTOR * _rewardAmount;
        ...
        uint256 _expectedDuration2 = _percentOf(_scaledRewardsAfterDuration1 / _stakeAmount1, _durationPercent2);
        ...
        uint256 _expectedDuration3 = _percentOf(
            _scaledRewardsAfterDuration1 / (_stakeAmount1 + _stakeAmount2),
            _durationPercent3
        );
...
contract UnclaimedReward is StakerRewardsTest {
    ...
    function testFuzz_CalculatesCorrectEarningsWhenASingleDepositorAssignsAClaimerAndStakesForFullDuration(
        ...
        (, Staker.DepositIdentifier _depositId) = _boundMintAndStake(_depositor, _stakeAmount, _delegatee, _claimer);
        // The contract is notified of a reward
        _mintTransferAndNotifyReward(_rewardAmount);
        // The full duration passes
        _jumpAheadByPercentOfRewardDuration(101);
    ...
    function testFuzz_CalculatesCorrectEarningsForASingleUserThatDepositsStakeForPartialDuration(
        ...
        assertLteWithinOnePercent(govStaker.unclaimedReward(_depositId), _percentOf(_rewardAmount, _durationPercent));
    }
    ...
    function testFuzz_CalculatesCorrectEarningsForASingleUserThatDepositsStakeForThePartialDurationAndClaims(
        ...
        assertLteWithinOnePercent(
            govStaker.unclaimedReward(_depositId),
            _percentOf(_rewardAmount, 100 - _durationPercent)
        );
    }
    ...
    function testFuzz_CalculatesCorrectEarningsForTwoUsersThatDepositEqualStakeForFullDuration(
        ...
        // A user deposits staking tokens
        (, Staker.DepositIdentifier _depositId1) = _boundMintAndStake(_depositor1, _stakeAmount, _delegatee);
        // Some time passes
        _jumpAhead(3000);
        // Another depositor deposits the same number of staking tokens
        (, Staker.DepositIdentifier _depositId2) = _boundMintAndStake(_depositor2, _stakeAmount, _delegatee);
        // The contract is notified of a reward
        _mintTransferAndNotifyReward(_rewardAmount);
        // The full duration passes
        _jumpAheadByPercentOfRewardDuration(101);
    ...
    function testFuzz_CalculatesCorrectEarningsForTwoUsersWhenOneStakesMorePartiallyThroughTheDuration(
        ...
        // the time
        uint256 _depositor1ExpectedEarnings = _percentOf(_percentOf(_rewardAmount, 50), 34) +
        ...
        uint256 _depositor2ExpectedEarnings = _percentOf(_percentOf(_rewardAmount, 50), 34) +
            _percentOf(_percentOf(_rewardAmount, 25), 66);
    ...
    function testFuzz_CalculatesCorrectEarningsWhenAUserStakesThroughTheDurationAndAnotherStakesPartially(
        ...
        // time
        uint256 _depositor1ExpectedEarnings = _percentOf(_rewardAmount, 66) +
            _percentOf(_percentOf(_rewardAmount, 50), 34);
    ...
    function testFuzz_CalculatesCorrectEarningsWhenTwoUsersDepositForPartialDurationsAndThereAreTwoRewards(
        ...
        uint256 _depositor1ExpectedEarnings = _percentOf(_rewardAmount1, 40) + _depositor2ExpectedEarnings;
    ...
    function testFuzz_CalculatesCorrectEarningsWhenTwoUsersDepositDifferentAmountsForPartialDurationsAndThereAreTwoRewards(
        ...
        _jumpAheadByPercentOfRewardDuration(25);
        // A user stakes some tokens
        (, Staker.DepositIdentifier _depositId1) = _boundMintAndStake(_depositor1, _stakeAmount1, _delegatee);
        // Another 40 percent of the duration time elapses
        _jumpAheadByPercentOfRewardDuration(40);
        // Another user stakes some tokens
        (, Staker.DepositIdentifier _depositId2) = _boundMintAndStake(_depositor2, _stakeAmount2, _delegatee);
        // Another quarter of the duration elapses
        _jumpAheadByPercentOfRewardDuration(25);
        // The contract receives another reward, resetting the duration
        _mintTransferAndNotifyReward(_rewardAmount2);
        ...
        uint256 _depositor2ExpectedEarnings = (_stakeAmount2 * _combinedPhaseExpectedTotalRewards) / _combinedStake;
        ...
        uint256 _depositor1ExpectedEarnings = _percentOf(_rewardAmount1, 40) +
            (_stakeAmount1 * _combinedPhaseExpectedTotalRewards) /
            _combinedStake;
    ...
    function testFuzz_CalculatesCorrectEarningsWhenAUserDepositsAndThereAreThreeRewards(
        ...
        uint256 _depositorExpectedEarnings = _percentOf(_rewardAmount1, 40) +
            ...
            _percentOf(_percentOf(_percentOf(_rewardAmount1, 60) + _rewardAmount2, 70) + _rewardAmount3, 30);
        assertLteWithinOnePercent(govStaker.unclaimedReward(_depositId), _depositorExpectedEarnings);
    }
    ...
    function testFuzz_CalculatesCorrectEarningsWhenTwoUsersDepositForPartialDurationsAndThereAreThreeRewards(
        ...
        uint256 _depositor2ExpectedEarnings = _percentOf(_percentOf(_rewardAmount1, 20), 50) +
            ...
            _percentOf(
                _percentOf(_percentOf(_percentOf(_rewardAmount1, 35) + _rewardAmount2, 80) + _rewardAmount3, 20),
                50
            );
        ...
        // // earned were from the first reward notification.
        uint256 _depositor1ExpectedEarnings = _percentOf(_rewardAmount1, 20) + _depositor2ExpectedEarnings;
    ...
    function test_CalculatesEarningsInAWayThatMitigatesRewardGriefing() public {
        ...
        // be ~the same.
        assertLteWithinOnePercent(govStaker.unclaimedReward(_depositId1), govStaker.unclaimedReward(_depositId2));
    }
}
...
contract ClaimReward is StakerRewardsTest {
    ...
    function testFuzz_SendsTheClaimFeeToTheFeeCollector(
        ...
        vm.assume(
            _depositor != address(govStaker) &&
                _feeCollector != address(govStaker) &&
                _feeCollector != address(0) &&
                _depositor != _feeCollector
        );
    ...
    function testFuzz_RevertIf_TheCallerIsNotTheDepositClaimerOrOwner(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Staker.Staker__Unauthorized.selector, bytes32("not claimer or owner"), _notClaimer)
        );
        govStaker.claimReward(_depositId);
    }
...
contract Multicall is StakerRewardsTest {
    function _encodeStake(address _delegatee, uint256 _stakeAmount) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(bytes4(keccak256("stake(uint256,address)")), _stakeAmount, _delegatee);
    }

    function _encodeStake(
        address _delegatee,
        uint256 _stakeAmount,
        address _claimer
        ...
        return
            abi.encodeWithSelector(
                bytes4(keccak256("stake(uint256,address,address)")),
                _stakeAmount,
                _delegatee,
                _claimer
            );
    ...
    function _encodeStakeMore(
        Staker.DepositIdentifier _depositId,
        uint256 _stakeAmount
    ) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(bytes4(keccak256("stakeMore(uint256,uint256)")), _depositId, _stakeAmount);
    }
    ...
    function _encodeWithdraw(
        ...
        return abi.encodeWithSelector(bytes4(keccak256("withdraw(uint256,uint256)")), _depositId, _amount);
    }

    function _encodeAlterClaimer(
        Staker.DepositIdentifier _depositId,
        address _claimer
    ) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(bytes4(keccak256("alterClaimer(uint256,address)")), _depositId, _claimer);
    }

    function _encodeAlterDelegatee(
        Staker.DepositIdentifier _depositId,
        address _delegatee
    ) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(bytes4(keccak256("alterDelegatee(uint256,address)")), _depositId, _delegatee);
    }
    ...
    function testFuzz_CanUseMulticallToStakeAndAlterClaimerAndDelegatee(
        ...
        vm.assume(
            _depositor != address(0) &&
                _delegatee0 != address(0) &&
                _delegatee1 != address(0) &&
                _claimer0 != address(0) &&
                _claimer1 != address(0)
        );
        ...
        (uint96 _amountResult, , , address _delegateeResult, address _claimerResult, , ) = govStaker.deposits(
            _depositId
        );
