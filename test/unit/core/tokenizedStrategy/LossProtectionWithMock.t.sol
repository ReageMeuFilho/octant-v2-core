...
contract LossProtectionWithMockTest is Test {
    ...
    function setUp() public {
        ...
        strategyAllowDeposits = MockTokenizedStrategyWithLoss(address(new ERC1967Proxy(address(implementation), "")));
    ...
    function testFuzzSafeDepositSlippageProtection(uint256 depositAmount, uint256 slippageBps) public {
    ...
    function testFuzzSafeMintSlippageProtection(uint256 sharesToMint, uint256 slippageBps) public {
        // Bound inputs
    ...
    function testFuzzSlippageProtectionWithLoss(uint256 lossAmount, uint256 depositAmount, uint256 slippageBps) public {
        // Bound inputs
    ...
    function testFuzzSafeDepositSlippageReverts(uint256 depositAmount, uint256 minSharesOutMultiplier) public {
        // Bound inputs
        depositAmount = bound(depositAmount, 1e15, 1000e18);
    ...
    function testFuzzSafeMintSlippageReverts(uint256 sharesToMint, uint256 maxAssetsMultiplier) public {
        // Bound inputs
        sharesToMint = bound(sharesToMint, 1e15, 1000e18);
    ...
    function testFuzzDepositRevertsDuringLoss(uint256 lossAmount, uint256 depositAmount) public {
        // Bound inputs
    ...
    function testFuzzMintRevertsDuringLoss(uint256 lossAmount, uint256 sharesToMint) public {
        // Bound inputs
        lossAmount = bound(lossAmount, 1, 1000e18); // Any non-zero loss
        sharesToMint = bound(sharesToMint, 1e15, 1000e18);
    ...
    function testFuzzMaxDepositBehaviorWithLoss(uint256 lossAmount) public {
        // Bound inputs
        lossAmount = bound(lossAmount, 1, 1000e18); // Any non-zero loss
    ...
    function testFuzzMaxMintBehaviorWithLoss(uint256 lossAmount) public {
        // Bound inputs
        lossAmount = bound(lossAmount, 1, 1000e18); // Any non-zero loss
    ...
    function testFuzzSlippageProtectionEdgeCases(uint256 depositAmount, uint256 slippageBps) public {
    ...
    function testFuzzZeroSlippageTolerance(uint256 depositAmount) public {
        // Bound inputs
        depositAmount = bound(depositAmount, 1e15, 1000e18);
    ...
    function testFuzzComplexLossScenarios(
        ...
        if (expectedShares > 0 && lossAmount > 0 && strategyAllowDeposits.totalSupply() > 0) {
            assertTrue(
                actualShares <= newDepositAmount,
                "Should not receive more shares than deposited assets with loss"
            );
        }
