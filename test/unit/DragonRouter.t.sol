...
contract DragonRouterTest is Test {
    ...
    function test_addStrategy_reverts_alreadyAdded() public {
        ...
        bool found = false;
        for (uint256 i = 0; i < 10; i++) { // Check up to 10 strategies to be safe
            try routerTesting.strategies(i) returns (address strat) {
    ...
    function test_setTransformer() public {
        ...
        (,,,DragonRouter.Transformer memory userTransformer,) = routerTesting.userData(userWithBalance, strategies[0]);
    ...
    function test_claimSplit_reverts_insufficientBalance() public {
        ...
        vm.mockCall(
            testAsset,
            abi.encodeWithSelector(IERC20.transfer.selector, user, 100),
            abi.encode(true)
        );
    ...
    function test_updateUserSplit() public {
        ...
        (uint256 actualAssets, uint256 actualUserAssetPerShare,,,) = routerTesting.userData(userToUpdate, strategies[0]);
        
        // Check that userAssetPerShare was updated correctly
        assertEq(actualUserAssetPerShare, assetPerShare, "UserAssetPerShare should be updated to current assetPerShare");
    ...
    function test_transferSplit_withTransformer() public {
        ...
        vm.mockCall(
            asset,
            abi.encodeWithSelector(IERC20.approve.selector, transformerAddr, amount),
            abi.encode(true)
        );
    ...
    function test_claimableAssets() public {
        ...
        uint256 expectedClaimable = (userSplitPerShare * stratTotalShares * (stratAssetPerShare - userAssetPerShare)) / 1e18;
        ...
        assertTrue(
            claimable >= expectedClaimable - tolerance && 
            claimable <= expectedClaimable + tolerance,
