...
contract RecipientVerificationTest is Test {
    ...
    function test_RecipientVerification_ZeroAddress() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TokenizedAllocationMechanism.InvalidRecipient.selector, address(0))
        );
    ...
    function test_RecipientMismatchErrorFormat() public {
        ...
        vm.prank(alice);
        _tokenized(address(mechanism)).castVote(
            pid, TokenizedAllocationMechanism.VoteType.For, 10, recipientB
        );
    }
    ...
    function test_QuadraticVotingConstraints() public {
        ...
        vm.prank(alice);
        _tokenized(address(mechanism)).castVote(
            pid, TokenizedAllocationMechanism.VoteType.For, 10, recipientA
        );

        // Against vote should fail due to QuadraticVoting constraints
        vm.expectRevert(
            abi.encodeWithSelector(QuadraticVotingMechanism.OnlyForVotesSupported.selector)
        );
        vm.prank(alice);
        _tokenized(address(mechanism)).castVote(
            pid, TokenizedAllocationMechanism.VoteType.Against, 5, recipientA
        );
    }
    ...
    function test_E2E_RecipientVerification() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TokenizedAllocationMechanism.RecipientMismatch.selector, pid1, recipientB, recipientA)
        );
    ...
    function test_ReorganizationAttackPrevention() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TokenizedAllocationMechanism.RecipientMismatch.selector, pid2, recipientA, recipientB)
        );
